...
func (r *Rank) GroupByTime(ctx context.Context, rankLimit int, queryFuncs ...QueryOptFunc) (res []model.RankTimeGroup, err error) {
	...
	err = r.db.WithContext(ctx).Table("(?) AS group_rank", r.model(ctx).
		Select("score_id, foreign_id, extra, DATE_TRUNC('week',created_at) AS time, RANK() OVER ( PARTITION BY DATE_TRUNC('week',created_at) order by score*log(2, 1+hit) DESC, id ASC) AS rank").
		Scopes(opt.Scopes()...),
	).
