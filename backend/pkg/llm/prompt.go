...
- 避免简单重复已有回复，应提供补充信息
- 对于引用知识库回答的内容，请使用内联标记格式标注知识库的标题:
   - 标记格式：<span data-tooltip="<h3>来源</h3><br>标题">标签内容</span>
   - 如果一段回答引用了多个知识库内容，则使用组合标记格式：<span data-tooltip="<h3>来源</h3><br>标题1<br>标题2">标签内容</span>
   - 标记必须放在句子结尾
   - 标记的知识库不能进行修改
   - 标记中的 “来源“ 两个字和 html 标签 不能修改
   - 标记的标签内容为唯一知识库标题的数量，标签内容必须用中括号括起来，数量必须和引用的标题数量保持一致
