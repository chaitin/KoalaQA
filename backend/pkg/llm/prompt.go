...
- 对于引用到的知识库内容，请使用内联引用格式标注回答内容的标题:
   - 引用使用格式：<span data-tooltip="<h3>来源</h3><br>标题">1</span>
   - 如果一段回答引用了多个知识库内容，则使用组合引用格式：<span data-tooltip="<h3>来源</h3><br>标题1<br>标题2">[2]</span>
   - 句号前放置引用标记，没有句号则放在句子结尾
   - 引用的知识库内容不能进行修改
   - 标签内容为引用的知识库来源的数量，标签内容需要用中括号括起来，数量必须和引用的知识库内容数量保持一致
