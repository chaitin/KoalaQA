...
func (d *Discussion) LikeComment(ctx context.Context, userInfo model.UserInfo, discUUID string, commentID uint) error {
	...
	updated, stateChanged, err := d.in.CommLikeRepo.Like(ctx, disc.UUID, userInfo.UID, disc.ID, commentID, model.CommentLikeStateLike)
	if err != nil {
		return err
	}
	if !updated {
...
func (d *Discussion) DislikeComment(ctx context.Context, userInfo model.UserInfo, discUUID string, commentID uint) error {
	...
	updated, stateChanged, err := d.in.CommLikeRepo.Like(ctx, disc.UUID, userInfo.UID, disc.ID, commentID, model.CommentLikeStateDislike)
	if err != nil {
		return err
	}
	if !updated {
