...
const InfoRelevance = (props: { data: ModelDiscussionDetail }) => {
  ...
  const getAllHTag = () => {
    ...
    if (hash) {
      const target = headersWithLevel.find(
        (item) => item.text.replaceAll('\n', '') === hash
      );
      if (target) {
  ...
  return (
    ...
    >
      {markdownTitle.length > 0 && (
        ...
        >
          <Box sx={{ fontWeight: 600, fontSize: 16, color: '#000', mb: 2 }}>
            目录
          </Box>
          <Stack gap={1.5}>
            {markdownTitle.map((item) => (
              <Typography
                ...
                sx={{
                  ...
                  fontSize: 14,
                  color:
                    item.node === viewHTag ? 'primary.main' : 'rgba(0,0,0,0.8)',
                  ...
                  '&::before': {
                    ...
                    borderRadius: '50%',
                    backgroundColor:
                      item.node === viewHTag ? 'primary.main' : '#eee',
                  },
                }}
                onClick={() => {
