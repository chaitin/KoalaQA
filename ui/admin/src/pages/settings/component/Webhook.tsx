...
const Notification = () => {
  ...
  const columns: ColumnsType<ModelWebhook> = [
    ...
    },
    {
      ...
      dataIndex: "type",
      render: (text) =>
        notificationTypes.find((t) => t.value === text)?.label || text,
    },
    {
      title: "操作",
      width: 160,
  ...
  const onSubmit = async (data: ModelWebhook) => {
    try {
      if (editItem) {
        await putAdminSystemWebhookWebhookId(
          editItem.id!,
          data as SvcWebhookUpdateReq
        );
      } else {
                ...
                >
                  {contentOptions.map((option) => (
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          checked={(watch("msg_types") || []).includes(
                            option.value
                          )}
                          onChange={(e) => {
