          ...
          <Checkbox
            ...
            onChange={() => {
              if (selectIds.length === items.length) {
                ...
                setSelectIds(items.map(item => item.uuid || '').filter(Boolean));
              }
            }}
          />
          ...
          return (
            ...
            >
              {!task && showSelectAll ? (
                ...
                >
                  {task?.status === TopicTaskStatus.TaskStatusFailed ? (
                    <Icon type="icon-icon_tool_close" sx={{ fontSize: 18, color: 'error.main' }} />
                  ) : (
              ...
              <Box
                ...
                htmlFor={item.uuid}
              >
                <Ellipsis sx={{ fontSize: 14 }}>{item.docs?.[0].title}</Ellipsis>
                {item.docs?.[0].summary && (
              ...
              {[TopicTaskStatus.TaskStatusFailed, TopicTaskStatus.TaskStatusTimeout].includes(
                task?.status || TopicTaskStatus.TaskStatusCompleted
              ) && (
                <Button
                  size="small"
                  variant="outlined"
